...
def pop_up(msg):
    if popup in ['Victoire !','Perdu !','Génération :']:
        pygame.draw.rect(SCREEN, (0, 0, 0), (SW//2-200, SH //
                         2-160, 400, 320), border_radius=26)
        pygame.draw.rect(SCREEN, (15, 15, 15), (SW//2-196,
                         SH//2-156, 392, 312), border_radius=22)
        ...
        SCREEN.blit(texte[0], (SW//2-texte[1].width//2,
                    SH//2 - 130 - texte[1].height//2))
        #* Statistiques
        total = gamesize[0]*gamesize[1]
        texte = FONT.render('Statistiques :', (200, 200, 200))
        ...
        LITTLEFONT.render_to(SCREEN,(SW//2 - 180, SH//2 + 40),f'Difficulté : {int(f1(100*settings["difficulty"]))}',(150,150,150))

        LITTLEFONT.render_to(SCREEN,(SW//2 - 180, SH//2 + 70),f'Temps : {tf if popup in ["Victoire !", "Perdu !"] else int(time.time()-tps)}s',(150,150,150))

        #* Rejouer ?
        texte = FONT.render('Rejouer ?', (200, 200, 200))
        SCREEN.blit(texte[0], (SW//2+100-texte[1].width//2, SH//2 - 70 - texte[1].height//2))

        texte = LITTLEFONT.render('Largeur :', (150, 150, 150))
        SCREEN.blit(texte[0], (SW//2+100-texte[1].width//2, SH//2 - 40))

        texte = LITTLEFONT.render('Difficulté :', (150, 150, 150))
        SCREEN.blit(texte[0], (SW//2+100-texte[1].width//2, SH//2 + 30))
    else:
        pygame.draw.rect(SCREEN, (0, 0, 0), (SW//2-200, SH //
                         2-30, 400, 60), border_radius=26)
        pygame.draw.rect(SCREEN, (15, 15, 15), (SW//2-196,
                         SH//2-26, 392, 52), border_radius=22)
        texte = BOLD.render(msg, (255, 255, 255))
        SCREEN.blit(texte[0], (SW//2-texte[1].width//2,
                    SH//2 - texte[1].height//2))
...
def get_grid(src):
    ...
    for x in range(cx):
        for y in range(cy):
            if file[y][x] == -1:mines+=1
            tab[y][x] = file[y][x]
            Button(f"{x},{y}", (dx1 + size * x, dy1 + size * y),
                   (size, size), 'grid', str(int(file[y][x])), True)
    np.savetxt('newGrid.txt', tab, fmt="%.0f")
    find_start(tab,1)
    return tab,mines
...
while execute:
    ...
    if popup == '':
        ...
        if popup in ['Victoire !','Perdu !','Génération :']:
            if not 'playAgain' in Button.dict['classic']:
                Button('playAgain',(SW//2+20,SH//2+100),(160,40),'classic','Rejouer')
                Button('quit',(SW//2-180,SH//2+100),(160,40),'classic','Quitter')
    ...
    if (Button.counting == mines or (Button.wellmarked == mines and Button.marked == mines)) and popup != 'Perdu !' and tf == 0 and mines != gamesize[0]*gamesize[1]:
    ...
    for event in events:
        ...
        if event.type == MOUSEBUTTONUP:
            ...
            if event.button == 1:
                ...
                b = f'{Button.xp},{Button.yp}' in Button.dict[
                    'grid'] and not Button.dict['grid'][f'{Button.xp},{Button.yp}'].marked
                if b and popup == '':
        ...
        if event.type == KEYDOWN:
            if pressed_keys[K_ESCAPE]:
                if not popup in ['Victoire !','Perdu !','']:
                    popup = ''
                else:
                    execute = False
            ...
            if pressed_keys[K_g] and settings['dev']:
                tab,mines = get_grid('grid.txt')
                popup = ''
